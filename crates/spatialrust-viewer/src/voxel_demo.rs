...
impl Plugin for VoxelPlugin {
    fn build(&self, app: &mut App) {
        app.init_resource::<VoxelState>()
            ...
            .add_systems(
                Update,
                voxel_scene.run_if(in_state(AppDemo::Voxel)),
            );
    }
}
...
fn voxel_ui(mut contexts: EguiContexts, mut state: ResMut<VoxelState>) {
    ...
    egui::Panel::left("voxel_panel").default_size(250.0).show(ctx, |ui| {
        ...
        let c2 = ui.checkbox(&mut state.show_voxelized, "Voxelized (blue)").changed();
        if c1 || c2 {
...
fn voxel_scene(
    ...
    if state.needs_update {
        ...
        let vox = match VoxelGridDownsample::new(VoxelGridDownsampleConfig::centroid(state.voxel_size))
            .filter(&raw)
        {
            Ok(c) => c,
            Err(e) => {
        ...
        state.status = format!(
            "{} → {} pts ({:.1}×)",
            state.raw_count, state.vox_count,
            state.raw_count as f32 / state.vox_count.max(1) as f32
        );
    }
