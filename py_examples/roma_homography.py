...
def main():
    parser = argparse.ArgumentParser(description="RoMa + inlier homography example")
    parser.add_argument(
        "--img-a",
    ...
    parser.add_argument("--threshold", type=float, default=3.0, help="RANSAC threshold (px)")
    args = parser.parse_args()

    kpts_a, kpts_b = load_matches(Path(args.img_a), Path(args.img_b), args.device)
    result = inlier.estimate_homography_py(kpts_a, kpts_b, threshold=args.threshold)

    print("Homography result:")
